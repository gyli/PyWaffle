...
In the following example, we set `rows=5`, `values=[48, 46, 3]` and leave `comlums` empty. Then the block numbers would be the same to values. Since the sum of values is 97, the column number has to be 20 to fit all blocks.
